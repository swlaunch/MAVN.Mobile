...
BlocTester<PartnerPaymentsBloc> _blocTester =
    BlocTester(CompletedPartnerPaymentsBloc(_mockPartnerRepository));
...
void main() {
  group('PaginatedPartnerPaymentsBlocTests', () {
    setUp(() {
      ...
      _subject = CompletedPartnerPaymentsBloc(_mockPartnerRepository);
    ...
    test('get payments success', () async {
      when(
        _mockPartnerRepository.getCompletedPayments(
      ...
      verify(_mockPartnerRepository.getCompletedPayments(
    ...
    test('get payments generic error', () async {
      when(_mockPartnerRepository.getCompletedPayments(
        currentPage: anyNamed('currentPage'),
      )).thenThrow(Exception());
      ...
      _expectedFullBlocOutput.addAll([
        ...
        GenericListErrorState<PaymentRequestResponseModel>(
            error: LocalizedStrings.transferRequestListGenericError,
            currentPage: 1,
            list: []),
      ]);
